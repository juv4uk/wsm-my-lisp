...
- **my-lisp oracle**: власний `expr`/`expected` (або `error`) фікстури з `conformance.lisp`, без змін, авторитет my-lisp.
- **WSM/meta**: чи очікується, що `external/my-lisp/lib/meta-eval.lisp` (вихідний код самохостованого обчислювача) взагалі покриває цю форму — усі 17 є звичайним обчисленням, тому в цій колонці всюди `yes`; вона присутня структурно згідно з вимогами #4.
...
Усі 17 записів із тегом `compiler-corpus`, верифіковані за `external/my-lisp/tests/fixtures/conformance.lisp`:
